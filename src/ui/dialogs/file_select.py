...
class FileSelectDialog(QDialog):
    ...
    def __init__(self, files, selected_files, config=None, parent=None):
        super().__init__(parent)
        ...
        # 保存过滤配置
        self.skip_blank_lines = config.get('skip_blank_lines', True) if config else True
        self.skip_comment_lines = config.get('skip_comment_lines', True) if config else True
        self.comment_chars = config.get('comment_chars', []) if config else []
        self.encoding = config.get('encoding', 'auto') if config else 'auto'
        # 标题和提示
